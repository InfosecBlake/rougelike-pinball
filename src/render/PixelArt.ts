...
// ---------- Procedural shaded pixel disc (ball, torch base, rune tile) ----------
...
export function drawPixelDisc(
  ...
  const unit = Math.ceil((radius * 2) / gridSize);
  const half = gridSize / 2;
  for (let gy = 0; gy < gridSize; gy++) {
    for (let gx = 0; gx < gridSize; gx++) {
      const lx = (gx + 0.5 - half) / half;
      const ly = (gy + 0.5 - half) / half;
      const d = Math.hypot(lx, ly);
      if (d > 1) continue;
      const light = Math.max(0, -lx * 0.6 - ly * 0.75);
      const shadeAmt = light * 0.55 - d * 0.35;
      ctx.fillStyle = outline && d > 0.84 ? outline : shade(baseColor, shadeAmt);
      ctx.fillRect(Math.round(cx + lx * radius - unit / 2), Math.round(cy + ly * radius - unit / 2), unit, unit);
    }
  }
}

// ---------- Small pixel-grid sprite drawer ----------

/**
 * Draws a sprite from an ASCII grid: each string is one row, each character
 * maps to a palette color (or is skipped for '.'). Drawn centered on (0,0)
 * in the current transform, one grid cell = `unit` user-space units.
 */
export function drawPixelGrid(ctx: CanvasRenderingContext2D, rows: string[], palette: Record<string, string>, unit: number) {
  const h = rows.length;
  ...
  for (let y = 0; y < h; y++) {
    ...
    for (let x = 0; x < w; x++) {
      ...
      if (!color) continue;
      ctx.fillStyle = color;
      ctx.fillRect(Math.round(x * unit - offX), Math.round(y * unit - offY), unit, unit);
    }
  }
}
